...
### Docker + PostgreSQL + GitHub Actions (CI/CD) + Hostinger + Nginx + Gunicorn + Custom Domain + SSL
...
**Local → Docker → GitHub → Hostinger → Domain → HTTPS**
...
## Create Hostinger Server & SSH Key

👉 [Create a Hostinger account](https://rathank.com/hostinger/)

##### Create SSH Key
On your local machine:
```bash
ls ~/.ssh
ssh-keygen -t ed25519 -C "clickmart-hostinger"
```

Copy the public key and add it to Hostinger UI:
```ssh
cat ~/.ssh/hostinger.pub
```

## SSH into Hostinger (Passwordless)
```sh
...
```sh
VITE_SERVER_BASE_URL="http://<HOSTINGER_IP>:8000/api/v1"
```
...
## Create Environment Files on Hostinger
```sh
...
## Open Firewall Ports on Hostinger
...
Backend: http://<HOSTINGER_IP>:8000/

Frontend: http://<HOSTINGER_IP>:5173/
...
CORS_ALLOWED_ORIGINS = [
    'http://localhost:5173',
    'http://<HOSTINGER_IP>:5173'
]
```

In local .env.docker:
```sh
ALLOWED_HOSTS=<HOSTINGER_IP>,localhost,127.0.0.1
```

In hostinger .env.docker:
```sh
ALLOWED_HOSTS=<HOSTINGER_IP>,localhost,127.0.0.1
```
...
In docker-compose.yml:
```sh
VITE_SERVER_BASE_URL: "http://<HOSTINGER_IP>/api/v1"
```
...
### 🎯 Goal - Whenever I push code to GitHub, my Hostinger server should automatically update.

But first...

### Manually pull the code from GitHub to Hostinger.
While logged-in to Hostinger:
```sh
...
If you get error: Add ```backend``` to allowed host in hostinger server manually.
...
Connect Domain to Hostinger (DNS)
